...
> **架構決策**：PostgreSQL 正規化 Schema（方案 B），已部署在 **GCP Cloud Run + Cloud SQL**（正式環境，非規劃中）。存取模式採單純化設計——**查詢/報價完全公開**（任何人有連結就能用），**貼膜報價系統的系統設定/編輯功能需要密碼解鎖**；車型查詢小模組例外，**連寫入也完全公開、不需密碼**（現場人員要能隨時直接修改車款資料，見下方說明）。
...
- 車型查詢小模組：可依廠牌分頁或關鍵字搜尋車款，顯示車長與大小歸類（小車／一般車／大車／商用車）；**查詢、新增、編輯、刪除都不需要密碼**，任何人都能直接操作——這是刻意的設計，讓現場人員遇到系統沒有的車款時能馬上補上／修正，不需要先找人解鎖。資料同樣存在後台資料庫（`vehicle_brands`/`vehicles`），跟需要密碼保護的貼膜報價系統設定互不影響、互不共用權限。
