...
## 15. 進一步簡化：查詢公開、設定才要密碼（2026-07-31 追加）

實作完 Q1–Q4 版本後，使用情境進一步釐清：初期希望「單純化」——任何有連結的人都能直接查詢/報價，只有動到系統設定（成本、售價、折扣規則等）才需要密碼；因為目前只有極少數人會碰系統設定，不需要每個人都要帳號登入才能查詢。

因此對第 14 節的實作再做一次調整：

- **GET（查詢）一律公開**，不需要任何憑證——兩套工具打開就能直接使用，不再有強制登入畫面。
- **寫入（PUT／restore／import）維持需要憑證**，但改成「密碼解鎖」而非「帳號登入」：前端在使用者要新增/編輯/刪除車款、或進入「系統設定」頁時才跳出密碼框；新增 `POST /api/auth/unlock-settings {password}`，密碼比對邏輯是「符合任一 admin 帳號的密碼即可」，不需要輸入帳號名稱，解鎖後行為與原本 `admin` 角色完全相同（背後仍是同一組 JWT／`requireRole('admin')` 機制，只是拿掉了帳號欄位與強制登入頁）。
- `staff` 角色與 `POST /api/auth/login`（帳密登入）程式碼保留但目前前端沒有對應介面，之後若真的需要多組密碼分權限，再接上即可，不需要動資料庫 schema。
- 兩套前端工具的登入頁改為「解鎖」彈窗：右上角一顆鎖頭圖示，未解鎖時查詢/報價正常運作，點鎖頭或點「系統設定」才會要求密碼；解鎖後可再點一次鎖頭圖示鎖回去。

## 16. 車主自行擴充版本（v16／20260810）併入資料庫邏輯（2026-08-17 追加）

使用者在此文件規劃、後端建置完成後，自行（在別處）持續擴充了報價系統前端（檔名帶有 `v16_20260810`），新增了幾項在方案 B 上線當時的 Schema 沒有涵蓋的欄位。確認後將這些新增內容併入資料庫正規化邏輯，`frontend/quote-system-v14.html` 目前就是「v16 內容＋密碼解鎖後端整合層」的合併版本，作為使用者往後持續開發的基礎。

**新增/變更的資料表**（見 `backend/sql/schema.sql`）：

- `qs_vehicle_groups.usage_m` 拆成 `usage_rhino`／`usage_color` 兩欄——全車使用米數改為依產品類別（犀牛皮類／改色膜類）分別記錄，而非單一數字。
- 新增 `qs_op_codes`（OP代碼設定：整台車／局部指定部位／局部其他，各自的代碼與說明）與 `qs_op_code_specific_parts`（「局部指定部位」OP代碼適用的部位清單）。

**前端保留但後端不特別處理的部分**：v16 檔案裡的 `ensureUsageStructure()`／`ensureV14DataFill()`／`ensureFullPriceBonusRule()`／`ensureOpCodes()` 等一次性遷移函式，以及 `DB.meta.*` 旗標，是原本「純瀏覽器儲存」時代用來偵測「這個瀏覽器的舊資料要不要補欄位」的機制。併入資料庫版本後，這些函式原封不動保留在前端（當作「匯入舊版 JSON 備份檔」時的保護網，見 `importDBFromFile()`），但已將 `init()` 裡原本無條件執行的 `saveDB()` 改成「資料真的被這些函式改到、且目前是解鎖狀態才寫回後端」，避免一般訪客（未解鎖）每次開啟頁面都觸發一次注定失敗的寫入。

**驗證方式**：用真實 PostgreSQL 重新 migrate 套用新 schema，組出完整 v16 形狀的資料（含巢狀 usage、opCodes）透過 API 寫入再讀回，逐欄位比對確認完全一致；並用瀏覽器分別以「未解鎖訪客」「解鎖後管理者」跑過整台車報價／OP代碼設定頁的實際操作，確認不會出現非預期的錯誤提示、資料確實持久化到後端。
