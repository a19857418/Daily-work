...
## 存取模式：查詢公開、貼膜報價系統設定要密碼；車型查詢小模組連寫入都公開

初期採「單純化」設計：

- **查詢／報價完全公開**——任何拿到連結的人都能直接打開使用，不需要登入。
- **貼膜報價系統**：只有「寫入」（新增/編輯/刪除車款價格、系統設定裡的成本售價/折扣規則等）需要密碼：前端點下「系統設定」或編輯功能時會跳出密碼框，輸入正確密碼後才解鎖；密碼錯誤或沒輸入一律無法寫入。密碼共用一組即可（`backend/scripts/create-user.js` 建立的帳號，密碼比對時不需輸入帳號名稱，只要密碼對得上任一 admin 帳號就會解鎖）。
- **車型查詢小模組**：刻意設計成**連寫入也不需要密碼**——這是現場人員常用的工具，遇到系統沒有的車款要能馬上直接新增/修正大小分類，不應該卡密碼降低彈性。實作上是在 `server.js` 註冊路由時，對 `vehicleCatalog` 傳入 `makeSettingsRouter(repo, { publicWrite: true })`，讓 `PUT`／`restore`／`import` 都跳過 `requireAuth`/`requireRole('admin')`；`quoteSettings` 的路由沒有傳這個選項，維持原本需要密碼的行為。往後如果要新增其他模組，預設請沿用「寫入要密碼」，只有明確被要求「不需要密碼、要給現場彈性用」時才仿照車型查詢小模組傳 `publicWrite: true`。
